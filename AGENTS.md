# Dante Dashboard Workspace Instructions
...
- Use `rg` / `rg --files` for discovery, structured parsers for structured
  files, and `apply_patch` for manual edits.
- Parallelize independent read-only reads when useful.
